...
import { Check, ArrowRight, Activity, Users, Star, Lock, Heart, Clock, Utensils, Droplets, Zap, Dumbbell } from "lucide-react";
...
export function QuizContainer() {
  ...
  // Default state matches GeneratePersonalizedAfricanMethodPlanInput
  const [state, setState] = useState<GeneratePersonalizedAfricanMethodPlanInput>({
    name: "",
    age: 30,
    heightCm: 165,
    currentWeightKg: 75,
  ...
  const updateMealTime = (meal: keyof typeof state.mealTimes, value: string) => {
    setState((prev) => ({
      ...prev,
      mealTimes: { ...prev.mealTimes, [meal]: value },
    }));
  };
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              {["Férias", "Casamento", "Aniversário", "Reunião importante", "Outro", "Nenhum evento"].map((opt) => (
                <Button 
                  key={opt}
                ...
                <Card 
                  ...
                  onClick={() => {
                    ...
                    if (current.includes(opt as any)) {
                      ...
                      updateState("eventsLeadingToWeightGain", [...current, opt]);
                    }
                  }}
                >
      ...
      case 18:
        return (
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div>
                <p className="text-xs font-bold text-muted-foreground uppercase mb-2">COM CARNE</p>
                <div className="grid grid-cols-2 gap-2">
                  {["Tradicional", "Keto"].map(d => (
              ...
              <div>
                <p className="text-xs font-bold text-muted-foreground uppercase mb-2">SEM CARNE</p>
                <div className="grid grid-cols-2 gap-2">
                  {["Vegetariana", "Vegana"].map(d => (
                    <Button key={d} variant={state.preferredDietType.includes(d as any) ? "default" : "outline"} onClick={() => updateState("preferredDietType", [d])}>{d}</Button>
                  ))}
                </div>
              </div>
              <div>
                <p className="text-xs font-bold text-muted-foreground uppercase mb-2">OUTRAS</p>
                <div className="grid grid-cols-2 gap-2">
                  {["Mediterrânea", "Sem lactose", "Sem glúten"].map(d => (
                    <Button key={d} variant={state.preferredDietType.includes(d as any) ? "default" : "outline"} onClick={() => updateState("preferredDietType", [d])}>{d}</Button>
                  ))}
                </div>
              </div>
      ...
      case 21:
        return (
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              {["Entre 6h00 e 8h00", "Entre 8h00 e 10h00", "Entre 10h00 e meio-dia", "Eu normalmente pulo o café"].map(opt => (
    ...
    <div className="min-h-screen flex flex-col items-center bg-[#fafafa]">
      ...
      {currentStep < 43 && (
        <div className="fixed top-0 inset-x-0 z-50 bg-white/80 backdrop-blur-md px-4 py-3 flex items-center gap-4 border-b">
          <Button variant="ghost" size="icon" onClick={prevStep} disabled={currentStep === 1} className="shrink-0 h-8 w-8">
      ...
      {/* Main Content */}
      <div className={cn("w-full pt-16 pb-20 flex-1 flex flex-col items-center", currentStep >= 43 ? "pt-8" : "")}>
        <AnimatePresence mode="wait">
          <QuizStep key={currentStep} stepId={currentStep}>
            {renderStep()}
          </QuizStep>
        </AnimatePresence>
      </div>
